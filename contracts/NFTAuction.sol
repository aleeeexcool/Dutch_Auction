...
contract MyNFT {
    function tokenCashbackValues(uint256 tokenId, uint256 tokenPrice) public view virtual returns (uint256[] memory) {}

    function getCashbackAddress(uint256 tokenId) public view virtual returns (address) {}
}
...
contract NFTAuction is Ownable, Pausable {
    ...
    function _transferNFT(
        ...
        if (!isErc721) {
            ...
            if (_isTatumNFT(nftAddress, tokenId)) {
                if (MyNFT(nftAddress).getCashbackAddress(tokenId) == address(0)) {
                    uint256[] memory cashback = MyNFT(nftAddress)
    ...
    function bid(string memory id, uint256 bidValue) public payable whenNotPaused {
        ...
        if (newAuction.isErc721) {
            if (_isTatumNFT(newAuction.nftAddress, newAuction.tokenId)) {
                if (
                    MyNFT(newAuction.nftAddress).getCashbackAddress(
                        newAuction.tokenId
                    ) == address(0)
                ) {
                    uint256[] memory cashback = MyNFT(newAuction.nftAddress)
    ...
    function cancelAuction(string memory id) public payable virtual {
        ...
        if (_isTatumNFT(auction.nftAddress, auction.tokenId)) {
            if (
                MyNFT(auction.nftAddress).getCashbackAddress(auction.tokenId) ==
                address(0)
            ) {
                uint256[] memory cashback = MyNFT(auction.nftAddress)
